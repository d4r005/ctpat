...
export default function EmbarqueDetail() {
  ...
  const isAdmin = ['d.trujillo@brancoindustries.com', 'd4r005@gmail.com'].includes(user?.email || '') || user?.role === 'admin';
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg }}>
        <Section title={tr('almacen')}>
          <Row k={tr('almacenista')} v={t.almacenista} on={editMode ? (v: string) => setForm({...form, almacenista: v}) : null} />
          <Row k={tr('cliente')} v={t.cliente} on={editMode ? (v: string) => setForm({...form, cliente: v}) : null} />
        </Section>
        {/* ... etc ... */}
      </ScrollView>
    </SafeAreaView>
...
function Row({ k, v, on }: any) {
  return (
    <View style={styles.row}>
      ...
      {on ? <TextInput style={styles.input} value={v} onChangeText={on} /> : <Text style={styles.rowV}>{v}</Text>}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  editBtn: { backgroundColor: colors.brandSecondary, padding: 8 },
  secTitle: { backgroundColor: colors.brandPrimary, color: '#FFF', padding: 5, fontWeight: '900' },
  secBody: { borderWidth: 1, borderColor: colors.borderStrong },
  row: { flexDirection: 'row', padding: 10, borderBottomWidth: 1, borderBottomColor: '#EEE' },
  rowK: { width: 120, fontWeight: '700' },
  rowV: { flex: 1 },
  input: { flex: 1, backgroundColor: '#F9F9F9', padding: 2 }
});
