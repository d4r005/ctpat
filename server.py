...
def _detect_image_mime(img_data: bytes, declared: Optional[str] = None) -> str:
    ...
    if img_data[4:12] in (b"ftypheic", b"ftypheix", b"ftyphevc", b"ftypmif1"):
        return "image/heic"
    # Si no reconocemos los magic bytes, confiamos en lo declarado (si es un mime de imagen válido)
...
async def analyze_document_ai(image_b64: str, context: str, mime_type: Optional[str] = None) -> Dict[str, Any]:
    ...
    if real_mime == "image/heic":
        # Gemini no soporta HEIC (formato nativo de cámara de iPhone). El frontend debería
        # forzar JPEG en el picker, pero por si acaso avisamos con un error claro.
